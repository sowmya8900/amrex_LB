...
// // Define LIpair type for load-index pairs
...
// // Single implementation of LeastUsedCPUs
// void LeastUsedCPUs(int nprocs, std::vector<Long> bytes, Vector<int>& result, bool flag_verbose_mapper) {
//     result.resize(nprocs);

//     if (nprocs > 1) {
//         BL_PROFILE("DistributionMapping::LeastUsedCPUs()");

//         AMREX_ASSERT(nprocs == static_cast<int>(bytes.size()));

//         std::vector<LIpair> LIpairV;
//         LIpairV.reserve(nprocs);

//         // Create load-index pairs
//         for (int i = 0; i < nprocs; ++i) {
//             LIpairV.push_back(LIpair(bytes[i], i));
//         }

//         // Sort by workload (ascending)
//         std::sort(LIpairV.begin(), LIpairV.end(),
//                  [](const LIpair& a, const LIpair& b) { return a.first < b.first; });
...
//         if (flag_verbose_mapper) {
//             amrex::Print() << "LeastUsedCPUs sorted workloads:\n";
//             for (const auto& p : LIpairV) {
//                 amrex::Print() << "  Rank " << p.second << " contains " << p.first << "\n";
//             }
//         }
//     } else {
//         result[0] = 0;
